...
boot_alloc(uint32_t n)
{
	...
	// nextfree -> Virtual
	// n -> fisico
	// traducir de fisica a virtual para comparar
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *allocated_page = page_free_list;  // la agarro
	page_free_list = page_free_list->pp_link;          // avanzo la original

	allocated_page->pp_link =
	        NULL;  // para evitar doblefrees el siguiente del que agarre ahora es null
...
// Devuelve 0 si ocurrio un ERROR o 1 en caso de que se pudo completar la operacion.
int
create_page_table(pde_t *page_direc_entry)
{
	// Alloco una nueva pagina para la page table struct
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// la page direc en el indice contiene una direc fisica hay que pasarla
	// a a virtual ya que la mmu se encarga de pasarla a fisica
...
static void
boot_map_region_pages(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	size_t aux = 0;
	while (aux < size) {
...
#else
	// it can only be used in 22-bit aligned directions
	...
	while (aux_page < last_page) {
		...
		if ((uintptr_t) aux_page >= ULIM ||
			...
			if (aux_page < va) {
				...
				user_mem_check_addr = (uintptr_t) aux_page;
			}
			return -E_FAULT;
		}
		aux_page += PGSIZE;
