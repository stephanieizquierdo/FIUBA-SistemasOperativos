...
alloc_block(void)
{
	...
	for (uint32_t nb = 0; nb < super->s_nblocks; nb++) {
		if (block_is_free(nb)) {
			bitmap[nb / 32] &= ~(1 << (nb % 32));
			flush_block(bitmap);
			return nb;
		}
	}
...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if (!f->f_indirect) {
		...
		void *blockaddr = diskaddr(blockno);
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	*blk = diskaddr(*blkno);
	flush_block(blkno);  // nos corrigio que hace falta allocar pero esta alocado
	return 0;
}
