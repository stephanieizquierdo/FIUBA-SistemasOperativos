...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((result = openfile_lookup(envid, req->req_fileid, &o)) < 0) {
		return result;
	}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int size_buffer = PGSIZE - (sizeof(int) + sizeof(size_t));
	int req_n = req->req_n > size_buffer ? size_buffer : req->req_n;
